  ...
  if(hltConfig_.size() > 0){
	...
	triggerIndex_ = hltConfig_.triggerIndex(triggerNames_[itrig]);
      const unsigned int mIndex = triggerResultsHandle_->index(triggerIndex_);
       ...
       //cout << "even:" << iEvent.id() << "  Trigger index:"<<triggerIndex_ << "  mindex:"<< mIndex << "  trigger name:" << triggerNames_[itrig];
	...
	cout << "Filter index: "<<filterIndex << "  size:"<< trigFilterIndex.size() << "  Event handle:" << triggerEventHandle_->sizeFilters() << endl;
	
	if (filterIndex<triggerEventHandle_->sizeFilters()) {
	//cout <<"  Filter index: "<<filterIndex << "  filter size:"<< triggerEventHandle_->sizeFilters() << endl;
