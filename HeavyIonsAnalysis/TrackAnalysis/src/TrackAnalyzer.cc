...
TrackAnalyzer::beginJob()
{
  ...
  if(doMVA_){
   ...
   if(mvaSrcLabel_.label() == "generalTracks")    trackTree_->Branch("trkMVATight",&pev_.trkMVATight,"trkMVATight[nTrk]/O");  
  }
  if (doDebug_) {
  ...
  if (doSimTrack_) {
    ...
    if(fillSimTrack_){
 	...
 	trackTree_->Branch("mtrkMVA",&pev_.mtrkMVA,"mtrkMVA[nParticle]/F");
       ...
       if(mvaSrcLabel_.label() == "generalTracks")    trackTree_->Branch("mtrkMVATight",&pev_.mtrkMVATight,"mtrkMVATight[nTrk]/O");  
      }
      if (doPFMatching_) {
