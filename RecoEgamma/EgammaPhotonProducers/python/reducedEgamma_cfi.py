...
reducedEgamma = cms.EDProducer("ReducedEGProducer",
  keepPhotons = cms.string("hadTowOverEm()<0.15 && pt>0 && (pt>0 || chargedHadronIso()<10)"), #keep in output
  slimRelinkPhotons = cms.string("hadTowOverEm()<0.15 && pt>0 && (pt>0 || chargedHadronIso()<10)"), #keep only slimmed SuperCluster plus seed cluster
