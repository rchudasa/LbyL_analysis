...
void check_data_driven_id_eff_gen_trk( std::string infile_Forest = "data.txt",
			 std::string out = "test.root") 
{
  ...
  std::vector<float> gentrkP_1;
  std::vector<float> gentrkPt_1;
  std::vector<float> gentrkEta_1;
  std::vector<float> gentrkPhi_1;
 
  
  std::vector<float> gentrkP_2;
  std::vector<float> gentrkPt_2;
  std::vector<float> gentrkEta_2;
  std::vector<float> gentrkPhi_2;
  ...
  outputTreediPho->Branch("gentrkP_1",&gentrkP_1);
  outputTreediPho->Branch("gentrkPt_1",&gentrkPt_1);
  outputTreediPho->Branch("gentrkEta_1",&gentrkEta_1);
  outputTreediPho->Branch("gentrkPhi_1",&gentrkPhi_1);

  outputTreediPho->Branch("gentrkP_2",&gentrkP_2);
  outputTreediPho->Branch("gentrkPt_2",&gentrkPt_2);
  outputTreediPho->Branch("gentrkEta_2",&gentrkEta_2);
  outputTreediPho->Branch("gentrkPhi_2",&gentrkPhi_2);
  ...
  for (Long64_t j_entry=0; j_entry < entries; ++j_entry){
	...
	for(int i=0; i<ngenTrk; ++i){
	  for (int j=i+1; j<ngenTrk; ++j){
	    ...
	    if (gentrkcharge->at(i) == gentrkcharge->at(j)) continue;  
            if(gentrkPt->at(i) < 1.0 || gentrkPt->at(j) < 1.0) continue;
	   
	    
	    for(int k=0; k<nPho; ++k){

	      if(phoEt->at(k) < 1.0) continue;
	      ...
	      TLorentzVector v1, v2, v3, vSum;
	      v1.SetPtEtaPhiM( gentrkPt->at(i), gentrkEta->at(i), gentrkPhi->at(i), muMass);
	      v2.SetPtEtaPhiM( gentrkPt->at(j), gentrkEta->at(j), gentrkPhi->at(j), muMass);
	      v3.SetPtEtaPhiE( phoEt->at(k), phoEta->at(k), phoPhi->at(k), phoE->at(k));
	      vSum = v1+v2+v3;
	      
	      gentrkP_1.push_back(gentrkP->at(i));
	      gentrkPt_1.push_back(gentrkPt->at(i));
	      gentrkEta_1.push_back(gentrkEta->at(i));
	      gentrkPhi_1.push_back(gentrkPhi->at(i));
	      
	      gentrkP_2.push_back(gentrkP->at(j));
	      gentrkPt_2.push_back(gentrkPt->at(j));
	      gentrkEta_2.push_back(gentrkEta->at(j));
	      gentrkPhi_2.push_back(gentrkPhi->at(j));
	      ...
	      phoEt_1.push_back(phoEt->at(k));
	      phoEta_1.push_back(phoEta->at(k));
	      phoPhi_1.push_back(phoPhi->at(k));
	      
	      
	      if(dr1> 0.2 && dr2> 0.2 && vSum.Pt() < 1){
		cout << "Entry: "<< j_entry << "  Run:" << run << "  Event:" << event << "   lumi:" << lumis ;
		cout << "  nEle:" << nEle << "  muon:"<< nMu << "  Track 1 pt:" << gentrkP->at(i);
		cout << "  Track 2 pt:"<< gentrkP->at(j) <<"   Photon pt:"<< phoEt->at(k);// << endl;
...
{ 
  check_data_driven_id_eff_gen_trk(argv[1]);
