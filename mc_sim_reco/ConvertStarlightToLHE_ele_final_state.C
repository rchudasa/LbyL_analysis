...
void makeEventsFile(int num=10)
{
  string filename = "slight.out"; //
  ifstream infile(filename.c_str());
  char outfilename[1000];
  sprintf(outfilename,"qed_ee_pbpb_1GeV_ptcut_%d.lhe",num);
...
ofstream output(outfilename);
  ...
  while (getline(infile,temp_string)) {
	...
	charge = 1.;
      ...
      if(evt_n >=K && evt_n < K+M) 
	{
	  output << pdg_id << " 1 1 2 0 0 " << px << " " << py << " " << pz << " " << sqrt(ele*ele + px*px + py*py + pz*pz) << " " << ele << " 0. " << charge << endl;
...
void ConvertStarlightToLHE(int number_of_files=60) {
